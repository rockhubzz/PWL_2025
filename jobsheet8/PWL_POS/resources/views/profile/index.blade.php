...
@section('content')
    <div class="card card-outline card-primary">
        ...
        <div class="card-body">
            <div class="text-center mb-3">
                <a onclick="modalAction('{{ url('profile/update_foto') }}')" class="profile-pic-wrapper position-relative d-inline-block">
                    @if ($user->foto_profil)
                        <img src="{{ asset('pfp/' . (Auth::user()->foto_profil ?? 'default-profile.png')) }}" class="img-thumbnail rounded-circle profile-pic" width="150" height="150">
                    @else
                        <img src="{{ asset('pfp/default-profile.png') }}" class="img-thumbnail rounded-circle profile-pic" width="150">
                    @endif
        ...
        <div class="d-flex justify-content-center">
        <button onclick="modalAction('{{ url('user/' . $user->user_id . '/edit_ajax') }}')" class="btn btn-success" style="align-items: center">Ubah Data</button>
        </div>
    </div>
</div>
